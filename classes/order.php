...
class order
{
    ...
    public function add()
    {
        ...
        //Add new order
        $sql_insert_cart = "INSERT INTO orders VALUES(NULL,'$userId','" . date('Y-m-d H:i:s') . "',NULL,'Processing' )";
    ...
    function updateReceivedDateOrder($id)
    {
        ...
        $query = "UPDATE orders SET receivedDate = '" . date('Y-m-d H:i:s', strtotime('+3 days')) . "' WHERE id = $id";
    ...
    {
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $query = "UPDATE orders SET status = 'Complete', receivedDate = '" . date('Y-m-d H:i:s') . "' WHERE id = $id";
        $mysqli_result = $this->db->update($query);
        if ($mysqli_result) {
            return true;
        }
