...
class product
{
    ...
    public function insert($data)
    {
        ...
        $file_temps = $_FILES['image']['tmp_name'];
        $unique_images = array();
        
        foreach ($file_names as $key => $value) {
            $div = explode('.', $value);
            $file_ext = strtolower(end($div));
            $unique_image = substr(md5(time()), 0, 10) . '_' . $key . '.' . $file_ext;
            $uploaded_image = "uploads/" . $unique_image;
            move_uploaded_file($file_temps[$key], $uploaded_image);
            $unique_images[] = $unique_image;
        }
        
        $images = implode(',', $unique_images);

        
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $query = "INSERT INTO products VALUES (NULL,'$name','$originalPrice','$promotionPrice','$images'," . Session::get('userId') . ",'" . date('Y-m-d H:i:s') . "','$cateId','$qty','$des',1,0) ";
    ...
    public function getAllAdmin($page = 1, $total = 12)
    {
        if ($page <= 0) {
            $page = 1;
        }
    ...
    public function getCountPaging($row = 12)
    {
    ...
    public function getCountPagingClient($cateId, $row = 12)
    {
    ...
    public function getFeaturedProductss()
    {
        $tukhoa=$_POST['tukhoa'];
        $query =
            "SELECT *
			 FROM products WHERE name LIKE '%$tukhoa%'
			 WHERE products.status = 1
             order by products.soldCount DESC
             LIMIT 12";
        $result = $this->db->select($query);
    ...
    public function getProductsByCateId($page = 1, $cateId, $total = 12)
    {
        if ($page <= 0) {
            $page = 1;
        }
    ...
    public function searchProducts($search, $page = 1, $total = 12) {
        if ($page <= 0) {
            $page = 1;
        }
        $tmp = ($page - 1) * $total;
